...
O usuário pode marcar uma estação como favorita com uma estrela. Todas as estações favoritadas aparecem nessa tela em uma lista de cards. A dinâmica é simples: sem favoritos, lista vazia; com favoritos, cada card representa uma estação. O usuário gerencia seus favoritos de dentro de outras telas (ex: ao consultar uma rota ou mapa) — a My Stations é somente a visão consolidada do que foi salvo.

**Estado vazio:** quando não há favoritos, a lista é substituída por um componente de empty state com a mensagem:

> "Tu lista de favoritos está vacía, marca tu estación favorita"

Esse componente é local à screen — fica em `components/` dentro da pasta da própria screen, não é um componente compartilhado.
